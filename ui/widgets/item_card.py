...
class ItemCard(QFrame):
    ...
    def _setup_ui(self) -> None:
        ...
        # 2. Right Side: Top Action Buttons (Pin + More Options Menu)
        ...
        # Pin Button
        self._pin_button = QPushButton(self)
    ...
    def _create_text_content(self) -> QWidget:
        """Creates clean 2-3 line wrapped text preview."""
        container = QWidget(self)
        layout = QVBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(2)

        # Multi-line wrapped text (first 200 characters)
        raw_text = self.item.plain_text or self.item.title or "Empty clipboard item"
        lines = [line.strip() for line in raw_text.splitlines() if line.strip()]
        display_snippet = " \n".join(lines[:3]) if lines else raw_text

        if len(display_snippet) > 180:
            display_snippet = display_snippet[:180] + "..."
    ...
    def _load_thumbnail(self) -> None:
        """Loads and scales thumbnail image smoothly directly onto card."""
        if hasattr(self, "_image_preview"):
            ...
            if img_src:
                ...
                if not orig.isNull():
                    scaled = orig.scaled(
                        240, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation
                    )
